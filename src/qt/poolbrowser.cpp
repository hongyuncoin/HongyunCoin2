...
// Bitcoin to USD
const QString kCurrencyUSDUrl    = "http://blockchain.info/tobtc?currency=CNY&value=1";
...
// MintPal API urls
const QString kMintPalSummaryUrl    = "http://trade.jiuzhou168.info/API.php?type=ticker&symbol=BTC_CNY";
const QString kMintPalOrdersUrl     = "http://trade.jiuzhou168.info/API.php?type=rate_list&symbol=BTC_CNY&count=1";
const QString kMintPalHistoryUrl    = "http://trade.jiuzhou168.info/API.php?type=ex_rec&symbol=ksc_CNY&tid=10&count=80";
...
//                   && marketObject["MarketName"].toString() == "BTC-HYC") {
...
void PoolBrowser::parseMintPalSummary(QNetworkReply *replay)
{
    ...
    if (jsonResponse.isObject()) {
        ...
        if (dataObject.contains("last_price")) {
            ...
            // Saving variables needed to calculate form HYC to BC and USD
    ...
    if (jsonResponse.isObject()) {
        ...
        if (mainObject.contains("return")) {
            ...
            if (returnObject.contains("markets")) {
                ...
                if (marketsObject.contains("HYC")) {

                    QJsonObject HYCObject = marketsObject["HYC"].toObject();

                    // Updating summary labels
                    this->refreshStringVarAsBTCUsingField(HYCObject, "lasttradeprice",
                                                    lastCryptsy, ui->lastBTCCryptsyLabel);
                    lastCryptsy = this->refreshStringVarAsUSDUsingField(HYCObject, "lasttradeprice",
                                                                  lastCryptsy, ui->lastUSDCryptsyLabel);

                    this->refreshStringVarUsingField(HYCObject, "volume",
                                               volumeCryptsy, ui->volumesBTCCryptsyLabel);

//                    volumeCryptsy = this->refreshStringVarAsUSDUsingField(HYCObject, "volume",
//                                                                    volumeCryptsy, ui->volumeUSDCryptsyLabel);

                    double high = -999999999;
                    double low  =  999999999;
                    int i = 0;
                    if (HYCObject.contains("recenttrades")) {

                        //ui->tradesCryptsyTable->clear();

                        QJsonArray tradesArray = HYCObject["recenttrades"].toArray();
...
void PoolBrowser::parseCryptsyOrders(QNetworkReply *replay)
{
    ...
    if (jsonResponse.isObject()) {
        ...
        if (mainObject.contains("return")) {
            QJsonObject returnObject = mainObject["return"].toObject();
            if (returnObject.contains("HYC")) {
                QJsonObject HYCObject = returnObject["HYC"].toObject();

                int sellCount = 50, buyCount = 50;
                if (HYCObject.contains("sellorders")) {
                    sellCount = HYCObject["sellorders"].toArray().count();
                }
                if (HYCObject.contains("buyorders")) {
                    buyCount = HYCObject["buyorders"].toArray().count();
                }
                ...
                if (HYCObject.contains("sellorders")) {
                    ...
                    double ask = 9999999;
                    QJsonArray sellArray = HYCObject["sellorders"].toArray();
                    //ui->sellTableCryptsy->clear();
                ...
                if (HYCObject.contains("buyorders")) {
                    ...
                    QJsonArray sellArray = HYCObject["buyorders"].toArray();
                    //ui->buyTableCryptsy->clear();
                    int i = 0;
                    foreach (const QJsonValue &value, sellArray) {
