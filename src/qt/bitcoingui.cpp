    ...
    rpcConsole(0)
{
    ...
    setWindowTitle(tr("HongyunCoin2") + " " + tr("Wallet"));
    setObjectName("HongyunCoin2-qt");
    setStyleSheet("#HongyunCoin2-qt {background-color:#fbf9f6; font-family:'Open Sans';}");
   ...
   // actionConvertIcon = new QAction(QIcon(":/icons/changevalHongyunCoin2"), tr(""), this);
    ...
    // Progress bar and label for blocks download, disabled for current HongyunCoin2 wallet
    ...
    // When clicking the current currency logo, the currency will be converted into HongyunCoin2, BTC or USD
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setToolTip(tr("Send coins to a HongyunCoin2 address"));
...
//    tabGroup->addAction(poolAction);
    ...
    blockAction->setToolTip(tr("Explore the HongyunCoin2 blockchain"));
    ...
    statisticsAction->setToolTip(tr("View HongyunCoin2 statistics"));
    statisticsAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_6));
    statisticsAction->setCheckable(true);
    tabGroup->addAction(statisticsAction);
    
    optionsAction = new QAction(tr("&Settings"), this);
    optionsAction->setToolTip(tr("Modify settings for HongyunCoin2 wallet"));
   ...
   // connect(poolAction, SIGNAL(triggered()), this, SLOT(showNormalIfMinimized()));
    ...
    aboutAction = new QAction(QIcon(":/images/aboutHongyunCoin2"), tr("&About HongyunCoin2"), this);
    aboutAction->setToolTip(tr("Show information about HongyunCoin2"));
...
void BitcoinGUI::createToolBars()
{
   ...
   // quitBar->addAction(actionConvertIcon);
    ...
    navigationBar->addAction(statisticsAction);
...
void BitcoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if(clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("HongyunCoin2 client") + QString(" ") + tr("[testnet]"));
...
void BitcoinGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("HongyunCoin2 client"));
...
//    {
//        actionConvertIcon->setIcon(QIcon(":/icons/changevalHongyunCoin2").pixmap(STATUSBAR_wICONSIZE,STATUSBAR_hICONSIZE));
//        convertmode = 0;
//    }
//}
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to HongyunCoin2 network", "", count));
}
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid HongyunCoin2 address or malformed URI parameters."));
    }
